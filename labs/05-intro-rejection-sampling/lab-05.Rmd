...
According to the rejection sampling approach sample from f(x) using the Unif(0,1) pdf as an enveloping function. In order to do this, we write a general rejection sampling function that also allows us to plot the histograms for any simulation size. Finally, our function also allows us to look at task 4 quite easily. 
...
sim_fun <- function(f, envelope = "unif", par1 = 0, par2 = 1, n = 10^2, plot = TRUE){
  ...
  # density ratio.
  samples <- proposal[unif(n) < density_ratio]
...
# Task 5
In this case, the uniform and beta enveloping functions have very similar performance. 
